use color_eyre::eyre::{eyre, Result}; // For improved error handling and reporting
    ...
    async fn send_email(&self, recipient: &Email, subject: &str, content: &str) -> Result<()> {
        ...
        let request = self
            ...
            .header(
                ...
                self.authorization_token.expose_secret().trim(), // Securely expose the authorization token
            )
            .json(&request_body);

        // Send the request and include Postmark's response body on failure for easier debugging.
        ...
        if !response.status().is_success() {
            ...
            return Err(eyre!("Postmark returned {status}: {body}"));
        }
